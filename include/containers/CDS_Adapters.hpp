...
public:
    ...
    struct Traits : public cds::container::split_list::make_traits<
        cds::container::split_list::ordered_list_traits<
            cds::container::michael_list::make_traits<
                cds::opt::less<std::less<uint64_t>>
            >::type
        >
    >::type {};

    using map_type = cds::container::SplitListMap<cds::gc::HP, uint64_t, uint64_t, Traits>;
    ...
    bool do_find(uint64_t key, uint64_t& value) {
        return map_.find(key, [&value](auto const& pair) {
            value = pair.second;
        });
    }
...
public:
    ...
    using map_type = cds::container::FeldmanHashMap<cds::gc::HP, uint64_t, uint64_t, Traits>;
