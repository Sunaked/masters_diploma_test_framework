...
//
// Здесь приведён простой concurrent hash map на основе sharded std::unordered_map
// с per-shard std::shared_mutex (readers-writer lock). Это baseline для сравнения
// с lock-free реализациями.
// ============================================================================
...
public:
    bool do_insert(uint64_t key, uint64_t value) {
        ...
        if (!inserted) it->second = value; // upsert semantics
        return true;
    }
...
private:
    ...
    Shard& get_shard(uint64_t key) {
        ...
        size_t idx = static_cast<size_t>((key * kFibMul) >> (64 - __builtin_ctzll(NumShards)));
        return shards_[idx];
    }
};
