...
inline RunResult execute_run(
    ...
    CsvWriter&            csv)
{
    spdlog::info("  Run {}: {} threads, scenario '{}'",
                 run_index, thread_count, scenario.name);
    ...
    // Записываем в CSV
    csv.append_raw(rr);
    csv.write_system_metrics(rr.container_name, rr.scenario_name,
                             rr.thread_count, rr.run_index, samples);
