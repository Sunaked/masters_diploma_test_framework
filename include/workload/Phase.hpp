...
/// Одна предвычисленная операция (8 байт ключ + 1 байт тип = 9 байт, выравнивание до 16).
/// Компактное представление минимизирует cache pollution при итерации.
struct alignas(16) Op {
    uint64_t key;
    OpType   type;
};
static_assert(sizeof(Op) == 16, "Op should be 16 bytes for cache-line alignment");
