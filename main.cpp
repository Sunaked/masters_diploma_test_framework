...
std::vector<Scenario> load_scenarios(sol::state& lua) {
    ...
    for (size_t i = 1; i <= tbl.size(); ++i) {
        ...
        for (size_t j = 1; j <= phases.size(); ++j) {
            ...
            if (dist == "zipfian")       ph.key_dist = KeyDist::Zipfian;
            else if (dist == "sequential") ph.key_dist = KeyDist::Sequential;
            else                           ph.key_dist = KeyDist::Uniform;
...
std::vector<RunPlan> compute_run_plans(
    const std::vector<ContainerConfig>& containers,
    ...
    const GlobalConfig&                 global)
{
    ...
    for (const auto& cc : containers) {
        ...
        uint32_t num_configs = static_cast<uint32_t>(scenarios.size() * threads_list.size());
        ...
        if (avg_run_min <= 0) avg_run_min = 0.5; // fallback: 30 сек

        uint32_t max_total_runs = static_cast<uint32_t>(available_min / avg_run_min);

        // Распределяем равномерно по конфигурациям
        uint32_t runs_per_config = (num_configs > 0)
        ...
        if (estimated_total_min > available_min * 1.2) {
            spdlog::warn("Container '{}': estimated time {:.1f} min exceeds budget {:.1f} min. "
        ...
        spdlog::info("Container '{}': {} configs × {} runs = {} total runs "
                     ...
                     num_configs * runs_per_config,
                     available_min, runs_per_config * num_configs * avg_run_min);
...
int main(int argc, char* argv[]) {
    ...
    spdlog::info("Configuration loaded: {} containers, {} scenarios, {} thread counts",
    ...
    auto plans = compute_run_plans(container_cfg, scenarios, threads_list, global_cfg);
    ...
    for (const auto& plan : plans) {
        spdlog::info("=== Container: {} (type: {}) ===", plan.container_name, plan.container_type);
        ...
        for (const auto& cfg : plan.configs) {
            ...
            for (uint32_t run = 0; run < cfg.num_runs; ++run) {
                auto rr = execute_run(*container, *scenario,
                                      cfg.thread_count, run, exec_cfg, csv);

                std::string key = plan.container_name + "|"
                                + cfg.scenario_name + "|"
